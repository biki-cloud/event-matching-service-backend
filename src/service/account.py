...
from src.service.eventer import EventerModel, EventerDomain
...
class AccountDomain:
    ...
    def get_account(self, account_id: int = None) -> AccountModel:
    ...
    def get_access_token_for_login(self, form_data: OAuth2PasswordRequestForm = Depends()):
        ...
        """
        # DBにユーザーが存在するか確認
        email = form_data.username  # ユーザ名はemail. ほんとはusernameとかにしたいけど、FastAPIのOAuth2PasswordRequestFormがusername
        # とpasswordを要求するので、ここではusernameとしている
        account = AccountModel(**self.repository.get_account_by_email(email)[0])
        # DBになかったら認証エラーを返す
        if not account:
            raise HTTPException(
