...
class EventModel(BaseModel):
    id: Union[UUID, None] = Field(default=None, description="イベントID",
    ...
    eventer_id: Union[UUID, None] = Field(default=None, description="イベントID",
                                          example="123e4567-e89b-12d3-a456-426614174000")
...
def get_event_pynamo_model_from_event_model(event_model: EventModel) -> EventPynamoModel:
    return EventPynamoModel(
        id=event_model.id,
        name=event_model.name,
        state=event_model.state,
        info=event_model.info,
        eventer_id=event_model.eventer_id
    )
